...
namespace Content.Server._Orion.Administration.UI;
...
public sealed class SkillsAdminEui : BaseEui
{
    ...
    private void HandlePointsChanged(SkillsAdminEuiPointsChangedMessage message)
    {
        ...
        if (_entMan.TryGetComponent<ActorComponent>(_targetEntity, out var actor))
        {
            var messageKey = message.NewBonusPoints > totalBonusPoints
                ? "skills-admin-notify-points-added"
                : "skills-admin-notify-points-removed";

            _prayer.SendSubtleMessage(actor.PlayerSession, actor.PlayerSession, string.Empty, Loc.GetString(messageKey));
        }
    ...
    private void HandleResetAll()
    {
        ...
        if (_entMan.TryGetComponent<ActorComponent>(_targetEntity, out var actor))
            _prayer.SendSubtleMessage(actor.PlayerSession, actor.PlayerSession, string.Empty, Loc.GetString("skills-admin-notify-skills-reset"));
