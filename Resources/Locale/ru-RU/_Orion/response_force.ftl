response-force-call-announce = ВНИМАНИЕ! К вам на станцию отправляется Отряд Быстрого Реагирования. Ожидайте прибытия.
response-force-paid-call-announce = ВНИМАНИЕ! Мы не располагаем свободным отрядом быстрого реагирования, вашей станции был оплачен отряд наёмников. Ожидайте прибытия.

response-force-ert-green-alert-name = ОБР Зелёный
response-force-ert-green-alert-call-title = ОБР | Зелёный Код
response-force-ert-blue-alert-name = ОБР Синий
response-force-ert-blue-alert-call-title = ОБР | Синий Код
response-force-ert-amber-alert-name = ОБР Янтарный
response-force-ert-amber-alert-call-title = ОБР | Янтарный Код
response-force-ert-red-alert-name = ОБР Красный
response-force-ert-red-alert-call-title = ОБР | Красный Код
response-force-ert-gamma-alert-name = ОБР Гамма
response-force-ert-gamma-alert-call-title = ОБР | Гамма Код
response-force-ert-epsilon-alert-name = ОБР Эпсилон
response-force-ert-epsilon-alert-call-title = ОБР | Эпсилон Код

response-force-cburn-name = РХБЗЗ
response-force-cburn-call-title = РХБЗЗ

response-force-death-squad-name = Элитный Отряд
response-force-death-squad-call-title = Элитный Отряд

response-force-inteq-name = Интек
response-force-inteq-call-title = ЧВК Интек

response-force-inteq-strike-name = Ударный Отряд Интек
response-force-inteq-strike-call-title = ЧВК Интек

response-force-honk-squad-name = Хонк Отряд
response-force-honk-squad-call-title = Отдел Организации Праздничных Мероприятий

response-force-assistant-name = Ассистенты
response-force-assistant-call-title = Отдел Менеджмента Состояния Технических Туннелей
